...
export const AccountSelector: React.FC<AccountSelectorProps> = ({ onAddAccount }) => {
  const { account, accounts, activeAccountIndex, switchAccount } = useWallet();
  ...
  const handleViewExplorer = (address: string, e: React.MouseEvent) => {
    e.stopPropagation();
            ...
            {accounts.map((acc, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  ...
                  {/* Explorer button */}
                  <button
                    onClick={(e) => handleViewExplorer(acc.address, e)}
                    className="p-1.5 hover:bg-slate-600 rounded transition-colors"
